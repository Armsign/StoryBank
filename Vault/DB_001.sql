...
DROP TABLE IF EXISTS DEPOSIT_COMMENTS_ARCHIVE;
CREATE TABLE DEPOSIT_COMMENTS_ARCHIVE (
...
BEGIN
	...
	INSERT INTO DEPOSIT_COMMENTS_ARCHIVE
        SELECT COMMENT, COMMENT_BY, COMMENT_ON, DEPOSIT,
               ID, IS_INAPPROPRIATE, REVIEWED_BY, REVIEWED_ON
        FROM    DEPOSIT_COMMENTS
        WHERE   DEPOSIT IN (
        SELECT  ID
        FROM    DEPOSITS WHERE STORED_ON < NOW() - INTERVAL 3 DAY);        

	INSERT INTO DEPOSITS_ARCHIVE
	SELECT  ID, PROMPT_ID, TITLE, VISITOR_ID,
			STORED_BY, STORED_AS, STORED_AT, STORED_ON,
			AUDIO_TYPE, AUDIO_LENGTH, IS_PLAYABLE, IS_TRANSCRIBED,
			TRANSCRIPTION, CHARACTER_DESIGN, HAS_CONSENT, USE_EMAIL,
			REVIEWED_BY, REVIEWED_ON 
        FROM DEPOSITS WHERE STORED_ON < NOW() - INTERVAL 3 DAY;

        DELETE FROM DEPOSIT_METRICS WHERE DEPOSIT IN (
        SELECT  ID 
        FROM    DEPOSITS WHERE STORED_ON < NOW() - INTERVAL 3 DAY);

        DELETE FROM DEPOSIT_TAGS WHERE DEPOSIT IN (
        SELECT  ID 
        FROM    DEPOSITS WHERE STORED_ON < NOW() - INTERVAL 3 DAY);

	DELETE FROM DEPOSITS WHERE STORED_ON < NOW() - INTERVAL 3 DAY;
